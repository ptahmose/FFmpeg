...
#define STORE_BGR24_MMX(g_mask24l,g_mask24h) \
            "psrlq         $8, %%mm2    \n\t" \
            "psrlq         $8, %%mm3    \n\t" \
            "psrlq         $8, %%mm6    \n\t" \
            "psrlq         $8, %%mm7    \n\t" \
...
static inline void RENAME(rgb32tobgr24)(const uint8_t *src, uint8_t *dst, int src_size)
{
    ...
    while (s < mm_end) {
        __asm__ volatile(
            ...
            STORE_BGR24_MMX(%2,%3)
            :: "r"(dest), "r"(s),
             "m"(mask24l), "m"(mask24h), "m"(mask24hh),
             "m"(mask24hhh), "m"(mask24hhhh)
            :"memory");
        dest += 24;
        s += 32;
    }
...
static inline void RENAME(rgb15tobgr24)(const uint8_t *src, uint8_t *dst, int src_size)
{
    ...
    while (s < mm_end) {
        __asm__ volatile(
            ...
            :"=m"(*d)
            :"r"(s),"m"(mask15b),"m"(mask15g), "m"(mask15r), "m"(mmx_null), "m"(mul15_mid), "m"(mul15_hi)
            :"memory");
        /* borrowed 32 to 24 */
        __asm__ volatile(
            "movq       %%mm0, %%mm4    \n\t"
            ...
            :: "r"(d), "m"(*s),
             "m"(*s), "m"(mask24l), "m"(mask24h), "m"(mask24hh),
             "m"(mask24hhh), "m"(mask24hhhh)
            :"memory");
        d += 24;
        s += 8;
...
static inline void RENAME(rgb16tobgr24)(const uint8_t *src, uint8_t *dst, int src_size)
{
    ...
    while (s < mm_end) {
        __asm__ volatile(
            ...
            "por        %%mm5, %%mm3    \n\t"
            :"=m"(*d)
            :"r"(s),"m"(mask16b),"m"(mask16g),"m"(mask16r),"m"(mmx_null),"m"(mul15_mid),"m"(mul15_hi),"m"(mul16_mid)
            :"memory");
        /* borrowed 32 to 24 */
        __asm__ volatile(
            "movq       %%mm0, %%mm4    \n\t"
...
static inline void RENAME(rgb15to32)(const uint8_t *src, uint8_t *dst, int src_size)
{
    ...
    while (s < mm_end) {
        __asm__ volatile(
            ...
            "pmulhw        %6, %%mm2    \n\t"
            PACK_RGB32
            ::"r"(d),"r"(s),"m"(mask15b),"m"(mask15g),"m"(mask15r) ,"m"(mul15_mid),"m"(mul15_hi)
            :"memory");
        d += 16;
        s += 4;
    }
...
static inline void RENAME(rgb16to32)(const uint8_t *src, uint8_t *dst, int src_size)
{
    ...
    while (s < mm_end) {
        __asm__ volatile(
            ...
            "pmulhw        %6, %%mm2    \n\t"
            PACK_RGB32
            ::"r"(d),"r"(s),"m"(mask16b),"m"(mask16g),"m"(mask16r),"m"(mul15_mid),"m"(mul15_hi),"m"(mul16_mid)
            :"memory");
        d += 16;
        s += 4;
    }
    ...
    for (y=0; y<height-2; y+=2) {
        ...
        for (i=0; i<2; i++) {
            __asm__ volatile(
                ...
                "add                        $8,      %%"REG_a"  \n\t"
                " js                        1b                  \n\t"
                : : "r" (src+width*3), "r" (ydst+width), "g" ((x86_reg)-width), "r"(rgb2yuv), "m"(ff_w1111), "m"(ff_bgr2YOffset)
                : "%"REG_a, "%"REG_d
            );
            ydst += lumStride;
            src  += srcStride;
